...
gbtts: WavLM comes from ../wavlm (vendored); s3prl is not needed.
"""
...
# ---------------------------------------------------------------------------
# WavLM hidden states without s3prl (gbtts)
# ---------------------------------------------------------------------------

class _WavLMHiddenStates(nn.Module):
    """Same output as s3prl UpstreamExpert(...)(wavs)["hidden_states"] for WavLM:
    the input of each encoder layer (B, T, C) followed by the encoder output."""

    def __init__(self, cfg_dict):
        super().__init__()
        from ..wavlm.WavLM import WavLM, WavLMConfig
        self.cfg = WavLMConfig(cfg_dict)
        self.model = WavLM(self.cfg)
        self.model.feature_grad_mult = 0.0
        self.model.encoder.layerdrop = 0.0
        self._captured = []
        for layer in self.model.encoder.layers:
            layer.register_forward_hook(lambda m, inp, out: self._captured.append(inp[0].transpose(0, 1)))
        self.model.encoder.register_forward_hook(lambda m, inp, out: self._captured.append(out[0]))
    ...
    def forward(self, wavs):
        ...
        lengths = torch.LongTensor([len(w) for w in wavs]).to(device)
        padding_mask = ~torch.lt(torch.arange(int(lengths.max())).unsqueeze(0).to(device), lengths.unsqueeze(1))
        self._captured = []
        self.model.extract_features(pad_sequence(wavs, batch_first=True), padding_mask=padding_mask, mask=False)
        hidden, self._captured = self._captured, []
...
class ECAPA_TDNN(nn.Module):
    ...
    def __init__(self, feat_dim=1024, channels=512, emb_dim=256,
        ...
        if feat_type == "fbank" or feat_type == "mfcc":
            ...
            # gbtts: s3prl is not a dependency. _WavLMHiddenStates reproduces
            # s3prl's UpstreamExpert for this use (layer inputs + final encoder
            # output via forward hooks, input layer_norm when cfg.normalize) and
            # keeps the `model` attribute so the fine-tuned checkpoint keys
            # (feature_extract.model.*) load unchanged. Built from the tiny
            # config: every weight comes from the fine-tuned checkpoint.
            if config_path is None:
                raise ValueError("a WavLM config (wavlm_large_cfg.json or .pt) is required")
            ...
            self.feature_extract = _WavLMHiddenStates(cfg_dict)
